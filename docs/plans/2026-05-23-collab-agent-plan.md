...
**Tech Stack:** Next.js 15.x / pnpm 10 / TypeScript 5(strict) / Tailwind 4 / shadcn/ui / Lucide React / Tremor 3 / Nivo 0.87 / framer-motion 11 / LangGraph.js 0.4 / AI SDK v6 / @ai-sdk/gateway / Drizzle ORM 0.36 / @neondatabase/serverless 0.9 / @upstash/redis 1.x / @upstash/ratelimit 2.x / Zod 3.23 / Vitest 2 / Playwright 1.48
...
- [ ] **Step 2: 写 `vercel.ts`(显式锁定 maxDuration,防 Vercel 默认值变化)**
...
export const SEED_EVIDENCE_CARDS: SeedEvidenceCard[] = [
  // ≥ 10 条真实贴合 4 场景的证据(历史决议 / 小红书声量 / 行业报告 / 竞品 等)
  // 详见生成的 JSON,本 plan 不全列;但每条 full_content ≥ 200 字
  // ...(seed 时由 scripts/seed.ts 调 embedManyTexts 生成 embedding)
];
...
## Phase P4 — LangGraph 9 节点 + 主图(critical_path,11 task)
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (av.status === "paused_hitl") {
    ...
    if (hitl && hitl.auto_approve_at && new Date(hitl.auto_approve_at) < new Date()) {
      // 自动 resume (call LangGraph Command(resume=originalValue))
      const graph = buildConsensusGraph();
      await graph.updateState({ configurable: { thread_id: av.id } }, {}, "n3_l2_evidence");   // 简化:approve = 原状态继续
      await db.update(hitl_audit).set({ resumed_at: new Date(), resume_decision: "approve", resume_reason: "auto-approve(5min timeout)" }).where(eq(hitl_audit.id, hitl.id));
...
### Task P5.D: 批 5D — P06 + P07 Safety Center(19 端点)— **第二重批次**

**端点清单**(P06 = 3,P07 = 16,共 19;对照 api.md §5.6 + §5.7):

P06:`GET discussion-frame` / `PATCH items/:id` / `GET export?format=...`

P07:
- 面板 1:`GET /api/llm/provider-events`、`POST /api/llm/manual-degrade`
- 面板 2:`GET /api/proposals/:id/redaction-diff`、`POST /api/proposals/:id/restoration-map/download`
- 面板 3:`GET /api/analysis-versions/:id/confidence-stats`
- 面板 4:`GET /api/proposals/:id/versions`、`POST /api/proposals/:id/rollback`
- 面板 5:`GET /api/hitl/pending`、`POST /api/hitl/:checkpointId/takeover`(=resume 别名)
- 面板 6:`GET /api/audit-logs`、`GET /api/audit-logs/export`
- 面板 7:`POST /api/proposals/:id/reproducibility-runs/start`(5.7.12a 新增)+ `GET /api/reproducibility-check?rr_id=...` SSE + `POST /api/reproducibility-runs/:id/finalize` + `GET /api/reproducibility-runs/:id` + `GET /api/proposals/:id/reproducibility-runs`
- 面板 8:`GET /api/analysis-versions/:id/prompts` + `POST /api/analysis-versions/:id/prompts/ab-compare`(5.7.17 新增)

**Files:** 16 个 route.ts(部分共用目录)+ `tests/integration/api-p07-safety.test.ts`

- [ ] **Step 1: 代表性实现 — `POST /api/proposals/:id/reproducibility-runs/start`**
...
- [ ] **Step 2-3: 实现其余 15 个端点 + 集成测试 + commit**

代表关键点:
- **`POST /api/proposals/:id/rollback`**:创建新 av(copy 旧 av JSONB)+ `rollback_from_id` + **更新 proposals.current_analysis_version_id**(H-A-7)
- **`GET /api/audit-logs/export`**:CSV 流式输出 + metadata 敏感字段截断(LOW-2)
- **`POST /api/analysis-versions/:id/prompts/ab-compare`**:用 `waitUntil` 异步触发后台 Haiku 推理,写入 av.methodology_ab_compare,响应 202 + estimated_ready_at
- **`POST /api/llm/manual-degrade`**:调 `setManualOverride` 写 Upstash Redis 300s TTL

```bash
git commit -m "feat(P5.D): 批 5D — P06 讨论框架 + P07 Safety Center 8 面板(19 端点)

- POST /reproducibility-runs/start 预创建 3 个 av(T 0.3/0.4/0.5,seed 42/84/126)
- POST /rollback 更新 current_analysis_version_id(H-A-7)
- POST /prompts/ab-compare 异步触发(B-A-3,避免阻塞 60s)
- audit-logs export CSV 流式 + 敏感字段截断
"
```

**acceptance_criteria:** 19 端点齐全;reproducibility-runs/start 预创建 3 个 av 入库;rollback 后 proposals.current 已更新
...
- [ ] **Step 2: 实现 P03 主页面 + 双层进度条 + Round 0 7 圆点 + framer-motion 动画**

略(代码模式同 P6.A,核心是消费 `useAnalysisStream` 状态渲染 ui.md §P03 组件)
...
git commit -m "feat(P7.4): Demo 前 30 分钟 Anthropic 配额预热脚本"
```
